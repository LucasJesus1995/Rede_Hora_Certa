...
class BI_BKP
{
    ...
    public static function BoletimProducaoAmbulatorial($params)
    {
        ...
        BI_BKP::$limite_competencia = Carbon::create($ano, $mes, 1, 0, 0, 0);
        ...
        foreach ($blocks AS $row) {
            ...
            $diff_realizacao = $realizacao_n->diffInMonths(BI_BKP::$limite_competencia);

            if ($diff_realizacao > 2) {
                $_realizacao = BI_BKP::$limite_competencia->format('Ym');
            } else {
        ...
        foreach ($blocks AS $cbo => $rows) {
            ...
            foreach ($rows AS $row) {
                ...
                //$block['cep'] = !empty($row['cep']) ? $row['cep'] : "04276000";
        ...
        $header['line'] = $totallinhas;// linhas
        $header['folha'] = $folhatotal; // folhas total
        ...
        $export = array_merge($_data['01'], $_data['02']);
        $export = array_merge($export, $_data['03']);
    ...
    private static function getCNES($cbo)
    {
        $cbos = self::$_cbos;

        return array_key_exists($cbo, $cbos) ? $cbos[$cbo][array_rand($cbos[$cbo])] : null;;
    }

    private static function getCnsNextByCbo($cbo)
    {
        $cbo_next = self::$_cbos_next;

        if (array_key_exists($cbo, $cbo_next)) {
            self::$_cbos_next[$cbo] += 1;
        } else {
            self::$_cbos_next[$cbo] = 0;
        }

        $indice = self::$_cbos_next[$cbo];
        if (!array_key_exists($indice, self::$_cbos[$cbo])) {
            $indice = 0;
            self::$_cbos_next[$cbo] = $indice;
        }

        return self::$_cbos[$cbo][$indice];
    }
