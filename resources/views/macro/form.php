...
Form::macro('fileField', function ($name, $label = null, $value = null, $attributes = array()) {
    $element = Form::file($name, $value, fieldAttributes($name, $attributes));
...
Form::macro('textTimeField', function ($name, $label = null, $value = null, $attributes = array()) {
    $element = Form::text($name, $value, fieldAttributes($name, $attributes));

    return fieldTimeWrapper($name, $element, $label);
});
...
Form::macro('checkboxField', function ($name, $displayName,  $value = 1, $checked = null, $attributes = array()) {
...
if (!function_exists('fieldTimeWrapper')) {

    function fieldTimeWrapper($field, $element, $label = null) {
        $out = '<div class="form-group input-group input-append bootstrap-timepicker';
...
if (!function_exists('fieldWrapper')) {
    ...
    function fieldWrapper($field, $element, $label = null) {
        ...
        if($label)
            $out .= fieldLabel($field, $label); // gen label
        //$out .= '<div class="col-md-12 md-form-group float-label">';
        $out .= $element;
        $out .= errorMessage($field); // display error message
        //$out .= '</div>';
        $out .= '</div>';
    ...
    function errorMessage($field) {
        if ($errors = Session::get('errors')) {
            if (!empty($errors->first($field)))
                return '<span class="label label-danger" style="display: block; text-align: left; font-weight: normal; margin-bottom: 10px; border-radius: 0; background-color: #A9434F">' . $errors->first($field) . '</span>';
        }
    }
...
if (!function_exists('fieldLabel')) {
    ...
    function fieldLabel($name, $label) {
        $out = '<label for="id-field-' . $name . '" class=" marginT10">';
        if ($label === null) {
