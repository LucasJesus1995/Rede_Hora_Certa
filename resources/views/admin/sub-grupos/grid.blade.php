            ...
            <tbody>
                @foreach($grid AS $row)
                    <tr class="grid-status-{{$row->ativo}}">
                        <td>{{$row->codigo_grupo}}.<strong>{{$row->codigo}}</strong></td>
                        <td>{{$row->descricaoGrupo}}</td>
                        <td>{{$row->descricao}}</td>
                        <td nowrap>
                            <a href="/admin/sub-grupos/entry/{{$row->id}}"  class="btn btn-rounded btn-xs btn-info waves-effect"><i class="fa fa-edit"></i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
