...
@if(!empty($report))
    ...
    @if(!empty($report))
        ...
        <table width="100%" border="1">
            <tr>
                <th colspan="{!! $colspan-10 !!}" class="title">TABELA DE ATENDIMENTO (GÊNERO)</th>
            ...
            @foreach($report AS $linha_cuidado => $profissionais)
                ...
                @foreach($profissionais AS $profissional => $rows)
                    <?php $letter = "D";?>
                    @foreach(['1'=>'Feminino','2' => 'Masculino'] AS $k => $sexo)
                        <tr class="line {!! $zebra =  ($color % 2 == "even") ? "odd" : "even" !!}">
                            ...
                            @for($i = 1; $i <= $ultimo_dia_mes; $i++)
                                <td class="center">
                                    @if(!empty($rows[$k]) && !empty($rows[$k][$i])) {!!  $rows[$k][$i] !!} @endif
                                </td>
                                <?php
                                if ($ultimo_dia_mes > $i) {
                                    $letter++;
                                ...
                                ?>
                            @endfor
                            <td class="center sub-title">=SUM(D{!! $line !!}:{!! $letter !!}{!! $line !!})</td>
                        </tr>
                        <?php $line++;?>
                    @endforeach
                    <?php $color++;?>
